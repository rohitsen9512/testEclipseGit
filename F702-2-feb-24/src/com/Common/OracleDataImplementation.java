...
public class OracleDataImplementation {
	...
	public static void main(String []arg,HttpServletRequest request){
	
		InserOracleDataInToPO(request);
	}
	
	public static void InserOracleDataInToPO(HttpServletRequest request)
	{
		...
		String query="SELECT * "+
" FROM OPENQUERY(ILPRD, 'select pha.segment1 PO_NUMBER,pha.creation_date PO_CREATION_DATE,aps.vendor_name,msi.segment1 item_name,plla.quantity,pla.unit_price,hla.location_code location ,pha.org_id, (plla.quantity*pla.unit_price) total_price"+
		...
		while(rs.next()){
			...
			if(rs1.next()){
			 ...
			 sql="insert into P_Purchase_Order(no_po, dt_po, id_ven, dt_approv, st_approv, approv_by, add_by, id_tax,tot, gr_tot) " +
					" values('"+po_number+"','"+po_creation_date+"',"+id_ven+",'"+po_creation_date+"','Accepted',1,1,1010,round("+total_price+",3,1),round("+total_price+",3,1))";
			 /*try{*/
			 ...
			 /*}catch(Exception e){
	            ...
	            query="insert into P_Purchase_Order_Asset(id_po,qty,rem_qty,un_prc,tot_prc,dt_recv,asset_consumable,description) values("+id_po+",round("+quantity+",3,1),round("+quantity+",3,1),round("+unit_price+",3,1),round("+total_price+",3,1),'"+po_creation_date+"','CapG', '"+assetName+"')";
	        	/*try{*/
	            con3=Common.GetConnection(request);
	            PreparedStatement ps2=con3.prepareStatement(query);
				ps2.executeUpdate();
	...
	private static String returnLocId(String LocName,HttpServletRequest request){
		...
		try{
			...
			if(rs4.next()){
				...
				sql = "inser into M_Loc(cd_loc, nm_loc, id_country)"+
					  " VALUES ('NA', '"+LocName+"', 1)";

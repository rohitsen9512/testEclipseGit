...
public class New_Ticket extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String action = "",search_state="",search_id_grp="",search_id_sgrp="",id="0",value="",ColName="",typeOfResult="",searchWord="";
		int json=0;
		...
		if(request.getParameter("search_id_grp") !=null)
		{
			search_id_grp = request.getParameter("search_id_grp");
		}
		if(request.getParameter("search_id_sgrp") !=null)
		{
			search_id_sgrp = request.getParameter("search_id_sgrp");
		}
			...
			switch (action)
	        {
	            ...
	            case "List":
	            	...
	            	if(typeOfResult.equals("Assigned_To"))
	            		sql="select id_ticket,t.id_grp,t.id_sgrp,ticket_no,priority,state,t.assign_grp,reqEmp.nm_emp as reqEmp,agnTo.nm_emp as agnEmp from HD_Tickets t,M_Emp_User reqEmp,M_Emp_User agnTo "+
	            				" where t.req_by=reqEmp.id_emp_user and t.assign_to=agnTo.id_emp_user and (t.state = 'New' or t.state ='Active' or t.state = 'Awaiting User Info') and t.assign_to="+id_emp_user+"";
	            	...
	            	sql="select id_ticket,t.id_grp,t.id_sgrp,ticket_no,priority,state,t.assign_grp,reqEmp.nm_emp as reqEmp from HD_Tickets t,M_Emp_User reqEmp "+
            				" where t.req_by=reqEmp.id_emp_user and t.state='Closed' ";
	            	
	            	else if(typeOfResult.equals("All"))
	            		//sql="select id_ticket,t.id_grp,t.id_sgrp,ticket_no,priority,state,reqEmp.nm_emp as reqEmp,agnTo.nm_emp as agnEmp,wh.id_wh_dyn as nm_prod from HD_Tickets t,M_Emp_User reqEmp,M_Emp_User agnTo,A_Ware_House wh "+
			            	//	" where t.req_by=reqEmp.id_emp_user and t.assign_to=agnTo.id_emp_user and t.id_prod=wh.id_wh";
	            		//sql="select * from HD_Tickets";
	            		sql="select id_ticket,t.id_grp,t.id_sgrp,ticket_no,priority,state,t.assign_grp,reqEmp.nm_emp as reqEmp from HD_Tickets t,M_Emp_User reqEmp "+
	            				" where t.req_by=reqEmp.id_emp_user order by id_ticket DESC ";
	            		...
	            		sql="select id_ticket,t.id_grp,t.id_sgrp,ticket_no,priority,state,t.assign_grp,reqEmp.nm_emp as reqEmp from HD_Tickets t,M_Emp_User reqEmp "+
	            				" where t.req_by=reqEmp.id_emp_user and t.state='Active' ";
	            	
	            	else if(typeOfResult.equals("Search_All")){

	            		String sqlTemp="";
	            		if(!search_state.equals("All")){
		            		if(!search_id_grp.equals("") && search_id_sgrp.equals(""))
		            			sqlTemp = " and t.state='"+search_state+"' and t.id_grp='"+search_id_grp+"' ";
		            		else if(search_id_grp.equals("") && search_id_sgrp.equals(""))
		            			sqlTemp = " and t.state='"+search_state+"' ";
		            		else if(!search_id_grp.equals("") && !search_id_sgrp.equals(""))
		            			sqlTemp = " and t.state='"+search_state+"' and t.id_grp='"+search_id_grp+"' and t.id_sgrp='"+search_id_sgrp+"' ";
		            	}else{
		            		if(!search_id_grp.equals("") && search_id_sgrp.equals(""))
		            			sqlTemp = " and t.id_grp='"+search_id_grp+"' ";
		            		else if(!search_id_grp.equals("") && !search_id_sgrp.equals(""))
		            			sqlTemp = " and t.id_grp='"+search_id_grp+"' and t.id_sgrp='"+search_id_sgrp+"' ";

	            		}
	            		
	            			
	            		
	            		sql="select (select count(*) from HD_Tickets t,M_Emp_User reqEmp where t.req_by=reqEmp.id_emp_user "+sqlTemp+" ) as tcount,id_ticket,t.id_grp,t.assign_grp,t.id_sgrp,ticket_no,priority,state,reqEmp.nm_emp as reqEmp from HD_Tickets t,M_Emp_User reqEmp "+
	            				" where t.req_by=reqEmp.id_emp_user "+sqlTemp+" ";
	            ...
	            case "Ticket_Dashboard":
	            	sql="select count(*) as totcount,state from HD_Tickets group by state ";
	            	jobj = uad.GetDataAsJsonFormate(sql,  request);
	...
	public JSONObject UpdateTicket(HashMap<String, String> map , String id,int id_emp_user,HttpServletRequest request)
	{
		...
		try 
		{
			...
			String priority = map.get("priority1");
...
//					hdm.SendEmailToAssignmentGroup(assign_grp,id_ticket,  request);
//				}
//			}
//			if(!assign_to.equals("")){
//				String oldAssign_to="";
...
//			}
			...
			query="update HD_Tickets set "+col_Val+" ,dt_closed=GETDATE() where id_ticket="+id+"";
			...
			String work_note = map.get("work_note");   
	         ...
	         if(!work_note.equals("")){
	     		...
	     		HelpDeskModel hdm = new HelpDeskModel();
				hdm.SendEmailToCreatorForTicketUpdate(work_note,id_ticket,"WIP", priority, request);
	     			     		
	         }
	         
	         String state = map.get("state");
	         if(state.equals("Closed") || state.equals("Cancelled") || state.equals("Closed as in complete")){
	         	
	        	 
	        	 HelpDeskModel hdm = new HelpDeskModel();
				hdm.SendEmailToCreatorForTicketUpdate(work_note,id_ticket,state, priority, request);
	         }

...
public class TransferHardwareRegisterSLMLOA
{
  public void transfer(String paramString1, String paramString2, 
		  String paramString3, ArrayList paramArrayList,HttpServletRequest request)
    throws Exception
  {
    ...
    String str11 = "ast_id,assetvalue,leasestatus,instl_date,leasestartdate,deprate,assetid,group_id,leaseenddate,start_date,end_date,asset_value";

    String str12 = "id_wh," + str4 + "," + "st_lease," + "" + str5 + "," + "std_lease," +  "depreciate," + "id_wh_dyn," + "id_grp," + "endt_lease," + "'" + paramString1 + "' as startdate," + "'" + paramString2 + "' as enddate,val_asst as asset_value";
    ...
    if (paramString3.equals("monthly")) {
      ...
      try
      {
        ...
        localStatement1.executeUpdate("update hr_dep set dep_startdate='" + paramString1 + "',dep_enddate='" + paramString2 + "' where start_date=convert(datetime,'" + paramString1 + "',101)");
      ...
      try
      {
        SLMLOAMonthDepCalcNUL localSLMLOAMonthDepCalcNUL = new SLMLOAMonthDepCalcNUL();
        localSLMLOAMonthDepCalcNUL.calculateDep( request);
        localSLMLOAMonthDepCalcNUL.add_del_calculateDep( request);
        localSLMLOAMonthDepCalcNUL.transfer_calculateDep(paramString1, paramString2,  request);
      ...
      try {
        ...
        localStatement1.executeUpdate("update hr_dep set deprate = d.cslm from hr_dep h, D_Dprn_Master d where d.id_grp = h.group_id and deprate = 1 ");
        localStatement1.executeUpdate("update hr_dep set previous_depamount=month_depreciation.depreciated_total,wdv=month_depreciation.wdv,dep_flag='EXISTING' from month_depreciation,hr_dep where hr_dep.assetid=month_depreciation.assetid and month_depreciation.enddate=convert(datetime,'" + paramString1 + "',101)-1");
        localStatement1.executeUpdate("delete from hr_dep where ast_id in (select id_wh from A_Ware_House a,addition_deletion b where a.id_wh=b.assetid and b.asset_status in('sold','writeoff') and a.id_wh  in  (select assetid from month_depreciation where type_id='CASLM' and status in('sold','writeoff')))");
      }
      ...
      try
      {
        SLMLOAMonthDepCalcUL localSLMLOAMonthDepCalcUL = new SLMLOAMonthDepCalcUL();
        localSLMLOAMonthDepCalcUL.calculateDep( request);
        localSLMLOAMonthDepCalcUL.add_del_calculateDep(  request);
        localSLMLOAMonthDepCalcUL.transfer_calculateDep(paramString1, paramString2,  request);
      } catch (Exception localException11) {
        System.out.println("Error 3" + localException11);
      }
      try {
        localStatement1.executeUpdate("insert into month_depreciation (type_id,group_id,ast_id,assetid,asset_opening,asset_addition,asset_deletion,asset_total,asondate,depreciated_opening,depreciated_addition,depreciated_deletion,depreciated_total,wdv,startdate,enddate,writeoff,status) select type_id,group_id,ast_id,assetid,asset_opening,asset_addition,asset_deletion,asset_total,asondate,depreciated_opening,depreciated_addition,depreciated_deletion,depreciated_total,wdv,'" + paramString1 + "','" + paramString2 + "',writeoff,status from month_depreciation where wdv=0 and asset_total > 0 and startdate=dateadd(month,-1,convert(datetime,'" + paramString1 + "')) and status not in ('sold','writeoff') and group_id in(" + str1 + ") and type_id='CASLM'");

        localStatement1.executeUpdate("update month_depreciation set asondate=depreciated_total,depreciated_opening=0.0,depreciated_addition=0.0,asset_opening=asset_total,asset_addition=0.0 where ast_id in (select ast_id from month_depreciation where wdv=0 and startdate=dateadd(month,-1,convert(datetime,'" + paramString1 + "'))) and startdate='" + paramString1 + "' and enddate='" + paramString2 + "'");
    ...
    } else if (paramString3.equals("yearly"))
    {
      ...
      try
      {
        SLMLOAYearlyDepCalcNUL localSLMLOAYearlyDepCalcNUL = new SLMLOAYearlyDepCalcNUL();
        localSLMLOAYearlyDepCalcNUL.calculateDep( request);
        localSLMLOAYearlyDepCalcNUL.add_del_calculateDep( request);
        localSLMLOAYearlyDepCalcNUL.transfer_calculateDep(paramString1, paramString2, request);
      ...
      try
      {
        SLMLOAYearlyDepCalcUL localSLMLOAYearlyDepCalcUL = new SLMLOAYearlyDepCalcUL();
        localSLMLOAYearlyDepCalcUL.calculateDep( request);
        localSLMLOAYearlyDepCalcUL.add_del_calculateDep( request);
        localSLMLOAYearlyDepCalcUL.transfer_calculateDep(paramString1, paramString2,  request);
      ...
      try {
        ...
        localStatement1.executeUpdate("update stline_depreciation set asondate=depreciated_total,depreciated_opening=0.0,depreciated_addition=0.0,asset_opening=asset_total,asset_addition=0.0 where ast_id in (select ast_id from stline_depreciation where wdv=0 and startdate=dateadd(year,-1,convert(datetime,'" + paramString1 + "'))) and startdate='" + paramString1 + "' and enddate='" + paramString2 + "'");

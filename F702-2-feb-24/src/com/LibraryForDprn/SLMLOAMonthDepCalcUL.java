...
public class SLMLOAMonthDepCalcUL
{
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      localStatement1.executeUpdate("update month_depreciation set depreciated_total=asset_total where wdv=0.0  and ast_id in(select id_wh from A_Ware_House where st_lease in ('Lease','UL'))");
      localStatement1.executeUpdate("update month_depreciation set depreciated_opening=0.0,depreciated_total=asset_total,wdv=0.0 where asondate=asset_total  and ast_id in(select id_wh from A_Ware_House where st_lease in ('Lease','UL'))");
    } catch (Exception localException2) {

...
public class MonthDepCalcUL
{
  ...
  public void add_del_calculateDep( HttpServletRequest request) throws Exception {
    ...
    str = "select assetid,instl_date,start_date,end_date,sold_writeoff_flag,assetvalue as assetaddition,asset_status_date,asset_status,case when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='add' and leaseenddate not between convert(datetime, asset_status_date,101) and convert(datetime, end_date,101) then  ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, end_date,101))+1)/(datediff(day,instl_date,leaseenddate)+1)))  when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='add' and leaseenddate between convert(datetime, asset_status_date,101) and convert(datetime, end_date,101) then  ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, leaseenddate,101))+1)/(datediff(day,instl_date,leaseenddate)+1)))  else  cast(0.0000 as decimal)  end as depreciated_addition,case when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='add' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date and leaseenddate not between convert(datetime,asset_status_date,101) and convert(datetime,sold_writeoff_date,101) then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,sold_writeoff_date,101))+1)/(datediff(day,asset_status_date,leaseenddate)+1)))  when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='add' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date and leaseenddate between convert(datetime,asset_status_date,101) and convert(datetime,sold_writeoff_date,101)  then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,leaseenddate,101))+1)/(datediff(day,asset_status_date,leaseenddate)+1)))  else  cast(0.0000 as decimal)  end as depreciated_addition_sold_writeoff,case when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='delete' and leaseenddate not between convert(datetime, asset_status_date,101) and convert(datetime, end_date,101) then  ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, end_date,101))+1)/(datediff(day,instl_date,leaseenddate)+1)))  when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='delete'  and leaseenddate between convert(datetime, asset_status_date,101) and convert(datetime, end_date,101) then  ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, leaseenddate,101))+1)/(datediff(day,instl_date,leaseenddate)+1)))  else  cast(0.0000 as decimal)  end as depreciated_deletion,case when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='delete' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date and leaseenddate not between convert(datetime,asset_status_date,101) and convert(datetime,sold_writeoff_date,101) then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,sold_writeoff_date,101))+1)/(datediff(day,asset_status_date,leaseenddate)+1)))  when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='delete' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date and leaseenddate between convert(datetime,asset_status_date,101) and convert(datetime,sold_writeoff_date,101) then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,leaseenddate,101))+1)/(datediff(day,asset_status_date,leaseenddate)+1)))  else  cast(0.0000 as decimal)  end as depreciated_deletion_sold_writeoff into temp_month_addition_deletion_dep from addition_deletion_dep";
    try
    {
      localStatement1.executeUpdate("drop table temp_month_addition_deletion_dep");
    } catch (Exception localException2) {
      System.out.println("\n UL Phase 1A: Problem occured during droping the temp_month_addition_deletion_dep table:\n" + localException2 + "\n...ends\n");
    }
    try {
      localStatement1.executeUpdate(str);
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      localStatement1.executeUpdate("update month_depreciation set depreciated_opening=(depreciated_opening-(Depreciated_total-asset_total)),wdv=0.0 where asset_total < = depreciated_total  and ast_id in(select id_wh from A_Ware_House where st_lease in ('Lease','UL'))");
      localResultSet1 = localStatement1.executeQuery("select distinct id_wh from A_Iut_History where dt_start between '" + paramString1 + "' and '" + paramString2 + "' and id_wh in(select ast_id from month_depreciation where startdate='" + paramString1 + "')");

     // localResultSet1 = localStatement1.executeQuery("select distinct id_wh from A_Iut_History where transferdate between '" + paramString1 + "' and '" + paramString2 + "' and ast_id in(select ast_id from month_depreciation where startdate='" + paramString1 + "')");
      while (localResultSet1.next()) {
        int i = 0;
        String str3 = localResultSet1.getString("ast_id");
        localResultSet2 = localStatement2.executeQuery("select * from A_Iut_History where id_wh='" + str3 + "' and dt_start between '" + paramString1 + "' and '" + paramString2 + "'");
        
       // localResultSet2 = localStatement2.executeQuery("select * from iut_history where ast_id='" + str3 + "' and transferdate between '" + paramString1 + "' and '" + paramString2 + "'");
        while (localResultSet2.next()) {
          i++;
        }
        if (i == 1) {
          localStatement3.executeUpdate("insert into cost_depreciation (asset_id,costcenter_code,costcenter_value,startdate,enddate) values()");
        }
      }
    }
    catch (Exception localException2)

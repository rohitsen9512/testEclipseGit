...
public class USMonthDepCalcNUL
{
    ...
    throws Exception
  {
    ...
    str = "select assetid,instl_date,dep_startdate,dep_enddate,sold_writeoff,group_id,previous_depamount,cast(0.0000 as decimal) as asset_deletion,cast(0.0000 as decimal) as depreciation_deletion,ast_id, CASE WHEN DEP_FLAG='NEW' then  assetvalue  else cast(0.0000 as decimal) end as assetaddition, CASE WHEN DEP_FLAG<>'NEW' then  assetvalue  else cast(0.0000 as decimal) end as assetopening, case when Sold_writeoff not in('WRITEOFF','SOLD') and  instl_date between convert(datetime, dep_startdate,101)  and  convert(datetime, dep_enddate,101) then ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, dep_enddate,101))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) when Sold_writeoff not in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101)  AND DEP_FLAG = 'NEW' then ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, dep_enddate,101))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) when Sold_writeoff not in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101) and dep_enddate >= dateadd(year,deprate,convert(datetime,instl_date,101)) AND DEP_FLAG = 'NEW' then ((AssetValue*(datediff(day,convert(datetime, instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) else cast(0.0000 as decimal)  end as depreciated_addition, case when Sold_writeoff not in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101)  AND DEP_FLAG <> 'NEW' then ( (AssetValue*(datediff(day,convert(datetime, dep_startdate,101),convert(datetime, dep_enddate,101))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) when Sold_writeoff not in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101)  AND DEP_FLAG <> 'NEW' and dep_enddate >= dateadd(year,deprate,convert(datetime,instl_date,101)) then ( (AssetValue*(datediff(day,convert(datetime, dep_startdate,101),dateadd(year,deprate,convert(datetime,instl_date,101)))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) else cast(0.0000 as decimal)  end as depreciated_opening,   case when Sold_writeoff in('WRITEOFF','SOLD') and  instl_date between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and sold__writeoff_date between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and sold__writeoff_date > instl_date then ((AssetValue*(datediff(day,convert(datetime,instl_date,101),convert(datetime,sold__writeoff_date,101))+1)/(datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))\t when Sold_writeoff in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101)  AND DEP_FLAG = 'NEW' and convert(datetime,sold__writeoff_date,101) < convert(datetime,dateadd(year,deprate,convert(datetime,instl_date,101)),101) then ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, sold__writeoff_date,101))+1) / (datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))\t when Sold_writeoff in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101)  AND DEP_FLAG = 'NEW'  and convert(datetime,sold__writeoff_date,101) > convert(datetime,dateadd(year,deprate,convert(datetime,instl_date,101)),101)  then ((AssetValue*(datediff(day,convert(datetime, instl_date,101),convert(datetime, dateadd(year,deprate,instl_date),101))+1) / (datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) else cast(0.0000 as decimal)  end as sold_writeoff_instl, case when Sold_writeoff in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101) and sold__writeoff_date between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and sold__writeoff_date > instl_date AND DEP_FLAG <> 'NEW' and convert(datetime, dateadd(year,deprate,instl_date),101) between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and convert(datetime, dateadd(year,deprate,instl_date),101) > convert(datetime,sold__writeoff_date,101) then  ((AssetValue*(datediff(day,convert(datetime,dep_startdate,101),convert(datetime,sold__writeoff_date,101))+1)/  (datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))when Sold_writeoff in('WRITEOFF','SOLD') and  instl_date <  convert(datetime, dep_startdate,101) and sold__writeoff_date between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and sold__writeoff_date > instl_date AND DEP_FLAG <> 'NEW' and convert(datetime, dateadd(year,deprate,instl_date),101) between convert(datetime, dep_startdate,101)  and convert(datetime, dep_enddate,101) and convert(datetime, dateadd(year,deprate,instl_date),101) < convert(datetime,sold__writeoff_date,101)  then  ((AssetValue*(datediff(day,convert(datetime,dep_startdate,101),convert(datetime, dateadd(year,deprate,instl_date),101))+1)/  (datediff(day,convert(datetime,instl_date,101),dateadd(year,deprate,convert(datetime,instl_date,101)))))) else cast(0.0000 as decimal)  end as sold_writeoff_startdate into temp_month_dep from hr_dep ";
    try
    {
  ...
  public void add_del_calculateDep( HttpServletRequest request) throws Exception {
    ...
    str = "select assetid,instl_date,start_date,end_date,sold_writeoff_flag,assetvalue as assetaddition,asset_status_date,asset_status,case when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='add' then  ((AssetValue*(datediff(day,convert(datetime, asset_status_date,101),convert(datetime, end_date,101))+1)/(datediff(day,convert(datetime,asset_status_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))  else  cast(0.0000 as decimal)  end as depreciated_addition,case when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='add' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,sold_writeoff_date,101))+1)/(datediff(day,convert(datetime,asset_status_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))  else  cast(0.0000 as decimal)  end as depreciated_addition_sold_writeoff,case when sold_writeoff_flag not in('WRITEOFF','SOLD') and asset_status='delete' then  ((AssetValue*(datediff(day,convert(datetime, asset_status_date,101),convert(datetime, end_date,101))+1)/(datediff(day,convert(datetime,asset_status_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))  else  cast(0.0000 as decimal)  end as depreciated_deletion,case when sold_writeoff_flag in('WRITEOFF','SOLD') and asset_status='delete' and sold_writeoff_date < convert(datetime, end_date,101) and sold_writeoff_date > asset_status_date then  ((AssetValue*(datediff(day,convert(datetime,asset_status_date,101),convert(datetime,sold_writeoff_date,101))+1)/(datediff(day,convert(datetime,asset_status_date,101),dateadd(year,deprate,convert(datetime,instl_date,101))))))  else  cast(0.0000 as decimal)  end as depreciated_deletion_sold_writeoff into temp_month_addition_deletion_dep from addition_deletion_dep";
    try
    {
    ...
    throws Exception
  {
    ...
    String str2 = "ast_id,'USG',group_id,assetid,assetopening,assetaddition,asset_deletion,(assetopening+assetaddition-asset_deletion),previous_depamount,(depreciated_opening+sold_writeoff_startdate),(depreciated_addition+sold_writeoff_instl),depreciation_deletion,(depreciated_opening+sold_writeoff_startdate+depreciated_addition+sold_writeoff_instl+previous_depamount-depreciation_deletion),((assetopening+assetaddition-asset_deletion)-(depreciated_opening+sold_writeoff_startdate+depreciated_addition+sold_writeoff_instl+previous_depamount-depreciation_deletion)),dep_startdate,dep_enddate,0,sold_writeoff";
    try
    {
      localStatement1.executeUpdate("insert into month_depreciation(" + str1 + ") select " + str2 + " from temp_month_dep");
      localStatement1.executeUpdate("update month_depreciation set depreciated_opening=(depreciated_opening-(Depreciated_total-asset_total)),wdv=0.0 where asset_total < = depreciated_total  and ast_id in(select ast_id from hardware_register where leasestatus in ('NoLease','NUL'))");
    } catch (Exception localException2) {

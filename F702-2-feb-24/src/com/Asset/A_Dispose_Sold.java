...
public class A_Dispose_Sold extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String paramValues = "";
		 ...
		 }catch(Exception e)
		 {
		  ...
		  int j=0;  
		String id="",action="",dt_sold="",uninstallAssetDate="",uninstallRmk="",id_flr="",id_cc="",to_assign="";
		...
		if(request.getParameter("DisposeAssetDate") !=null)
		{
			dt_sold= request.getParameter("DisposeAssetDate");
		}	
		String chk1[] = request.getParameterValues("uninstallAsset");
		System.out.println(dt_sold);
		try
		{
			...
			switch (action)
	        {
	            ...
	            case "Display":
	            	 tempSql="select distinct(id_wh_dyn),id_wh,ds_pro,no_mfr,nm_model,ds_asst,serial_no,nm_ven,wh.id_loc,(replace(convert(NVARCHAR, wh.dt_de_allocate, 103), ' ', '-')) dt_de_allocate from A_Ware_House wh , A_Invoice_master inv , M_Vendor v ,M_Model modl where (allocate = 7) and inv.id_inv_m=wh.id_inv_m and wh.id_model=modl.id_model and v.id_ven=inv.id_ven and st_trvl=0 ";
	            ...
	            case "Sold1":
	            	dt_sold = dateFormatNeeded.format(userDateFormat.parse(dt_sold));
	            	
	            	jobj = Sold(id,dt_sold);	
	                break;
	            case "Sold":
	            	...
	            	{
	            		String status="";
	            		dt_sold = request.getParameter("DisposeAssetDate"+chk1[i]);
	            		dt_sold = dateFormatNeeded.format(userDateFormat.parse(dt_sold));
		            ...
		            /*	String replyMailId="",name="",assetHolderId="",itemName="",slNo="",no_model="";
		    			...
		    			String mailBody = "<b>Hello "+name+" , </b><br><br><br>"+
		    						  ...
		    						  "<br><br>The <b>'"+itemName+"'</b> having Model No/Version/Version <b>'"+no_model+"'</b> ,Serial No <b>'"+slNo+"'</b> and asset id <b></b>.<br>"+
		    						  "<p>"+footerMsg+"</p>";
		    		
		    			Common.MailConfiguration(mailBody, replyMailId, recipients , mailSubject);
	            		*/
	            		
		    			String query =" update A_Ware_House  set sold='yes', allocate=3,dt_sold='"+dt_sold+"',to_assign = 0 , id_flr = 0 ,id_cc =0, dt_de_allocate = '"+uninstallAssetDate+"',rmk_asst='"+uninstallRmk+"' where id_wh = "+chk1[i]+"";
		    		System.out.println(query);
		    			...
		    			if(j>0)
		    			{
			    			...
			    			String	sql = "select to_assign,id_loc,id_subl,id_flr,id_cc from A_Ware_House where id_wh='"+chk1[i]+"'";
				    			...
				    			id_flr = rs.getString(4);
				    			id_cc = rs.getString(5);
				    			}
				    			 query = "insert into A_Iut_History(to_assign,id_loc,id_subl,id_flr,id_cc,id_wh,dt_de_allocate,type_tran,de_allocate_by,st_ins) values("+to_assign+","+id_loc+","+id_subl+","+id_flr+","+id_cc+","+chk1[i]+",'"+dt_sold+"','Intra',"+id_emp_user+",'Sold')";
	...
	public JSONObject Sold(String id,String dt_sold)
	{
		JSONObject jobj = new JSONObject();
		JSONObject json=new JSONObject();
		...
		int j=0;
		 ...
		 String sql1="update A_Ware_House set sold='yes', allocate=3,dt_sold='"+dt_sold+"' where id_wh ="+id+"";
		 try{
		 PreparedStatement ps=con.prepareStatement(sql1);

...
public class T_Inter_Accept_Reject extends HttpServlet {
			...
			switch (action)
	        {
	            ...
	            case "Display":
	        		...
	        		if(!UserType.equals("Super")){
	        			...
	        			tempSql= "select distinct(req_no),dt_req,nm_emp from A_Ware_House wh,A_Iut_History iut,M_Emp_User emp where  "+
		            			" wh.id_wh= iut.id_wh and iut_approval =1 and type_tran !='Intra' and emp.id_emp_user=wh.req_by and  acc_asst=0 and emp.id_emp_user=wh.req_by  and emp.repo_mngr="+id_emp_user+" "+tempQuery+" ";
	        		}
	        		else {
	        			tempSql= "select distinct(req_no),dt_req,nm_emp from A_Ware_House wh,A_Iut_History iut,M_Emp_User emp where  "+
    							" wh.id_wh= iut.id_wh and iut_approval =1 and type_tran !='Intra' and emp.id_emp_user=wh.req_by and  acc_asst=0  ";
	        		}
	        		/* tempSql= "select distinct(req_no),dt_req,nm_emp from A_Ware_House wh,A_Iut_History iut,M_Emp_User emp where (allocate=1 or allocate=2) and "+
							" wh.id_wh= iut.id_wh and iut_approval =1 and type_tran !='Intra' and emp.id_emp_user=wh.req_by and  acc_asst=0  ";
	        		 */
	            ...
	            case "GatePassPrint":
	        		...
	        		if(!UserType.equals("Super")){
	        			...
	        			tempSql= "select distinct(req_no),dt_req,nm_emp from A_Ware_House wh,A_Iut_History iut,M_Emp_User emp where  "+
		            			" wh.id_wh= iut.id_wh and (iut_approval =2 OR iut_approval =3 OR iut_approval =5) and type_tran !='Intra' and emp.id_emp_user=wh.req_by and  acc_asst=0 and wh.id_dept='"+id_dept+"' and emp.id_emp_user=wh.req_by and emp.repo_mngr="+id_emp_user+" Order by req_no DESC";
	        		}
	        		else {
	        			tempSql= "select distinct(req_no),dt_req,nm_emp from A_Ware_House wh,A_Iut_History iut,M_Emp_User emp where  "+
    							" wh.id_wh= iut.id_wh and (iut_approval =2 OR iut_approval =3 OR iut_approval =5) and type_tran !='Intra' and emp.id_emp_user=wh.req_by and  acc_asst=0  Order by req_no DESC";
	        		}			
	            ...
	            case "Preview":
	            	 dtoCommon = new DtoCommon();
	            	tempSql= "select id_wh,id_wh_dyn,ds_pro,serial_no,ds_asst from A_Ware_House where req_no="+req_no+" " ;
 ...
 ps=con.prepareStatement(sql);
	...
	public JSONObject UpdateTransferRequestForAcceptReject( String req_no , String status_approv,int id_emp_user,HttpServletRequest request)
	{
		...
		try 
		{
			...
			sql ="select tran_req_by,id_loc_tran,id_subl_tran,id_building_tran,id_flr_tran,id_div_tran from A_Iut_History where id_iut ="+id_iut+" and iut_approval=1 ";
			ps=con.prepareStatement(sql);
			rs = ps.executeQuery();
			if(rs.next())
			{
	    	...
	    	String Sql ="select id_emp from  M_Emp_User m where id_emp_user="+req_by+"";
		...
		if(!recipients.isEmpty())
		{
			...
			//Sql ="select id_emp from M_Emp_User e , M_User_Login u where e.id_emp_user=u.id_emp_user and  u.id_loc="+id_loc+"";
			...
			String mailBody = "Hello <b>Team</b>,<br><br><br>"+
					"<p>"+aprv+"</p><br>"+

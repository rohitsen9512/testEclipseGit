...
public class M_Mail_Config extends HttpServlet {
	...
	public JSONObject UpdateCompany(HashMap<String, String> map,String id,HttpServletRequest request)
	{
		...
		try
		{
				rs = Common.GetColumns("M_Mail_Config",  request);
				
						
						while (rs.next())
						{
						
							if(rs.getString(1) !="id_mail_config")
							{
		...
		String sql="select id_mail_config from M_Mail_Config";
		...
		rs=ps.executeQuery();
		    ...
		    if(rs.next())
		    {
		    	String query="update M_Mail_Config set "+col_Val+" where id_mail_config="+rs.getInt(1)+"";
		    ...
		    {
		    	String query="insert into M_Mail_Config("+colName+") values("+value+")";
		...
		{
			System.out.println("sql error in Company master."+e);
		}
		return json;
}
	...
	public JSONObject DisplayCompany()
	{
		JSONObject jobj = new JSONObject();
		JSONArray jarr = new JSONArray();
		String sql="select * from M_Mail_Config";

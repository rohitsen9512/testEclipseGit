...
public class Evaluate_RFI extends HttpServlet {
...
//           		 
	...
	public JSONObject DisplayReceiveQuotation(String id,String dt_frm,String dt_to,int id_emp_user,String word,String User_Type)
	{
		...
		*/
		...
		sql ="select sr.*,r.*,(replace(convert(NVARCHAR, dt_rfi, 103), ' ', '-')) as dtrfi,v.nm_ven from P_Submit_RFI sr,P_RFI r,M_Vendor v where sr.id_rfi=r.id_rfi and sr.id_ven=v.id_ven " ;
		}
		else{
			sql ="select sr.*,r.*,(replace(convert(NVARCHAR, dt_rfi, 103), ' ', '-')) as dtrfi,v.nm_ven from P_Submit_RFI sr,P_RFI r,M_Vendor v where sr.id_rfi=r.id_rfi and sr.id_ven=v.id_ven " ;

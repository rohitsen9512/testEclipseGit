...
public class Approval_Purchase_Order3 extends HttpServlet {
	...
	public JSONObject CheckApprovRejectDate(String dt_approv , String id_po)
	{
		...
		String query = "select (replace(convert(NVARCHAR, dt_approv, 103), ' ', '-')) as dt_approv2  from P_Purchase_Order where dt_approv2 >'"+dt_approv+"' and id_po='"+id_po+"'";
		try 
		{
	...
	public JSONObject PurchaseOrderDataForApproval(String id_po)
	{
		...
		try
		{
			...
			String sql="select emp.nm_emp,emp2.nm_emp as nm_emp2,po.*,poa.*,(replace(convert(NVARCHAR, po.dt_po, 103), ' ', '-')) as dtPo,(replace(convert(NVARCHAR, dt_recv, 103), ' ', '-')) as dtRecv,nm_prod,nm_curr,nm_tax from P_Purchase_Order po,P_Purchase_Order_Asset poa,M_Currency c,M_Prod_Cart pc,M_Tax t ,M_Emp_User emp,M_Emp_User emp2 "+
					" where t.id_tax=poa.id_tax and po.id_po=poa.id_po and po.id_curr=c.id_curr and poa.id_prod=pc.id_prod and po.id_po="+id_po+" and emp.id_emp_user=po.approv_by and emp2.id_emp_user=po.approv_by2";
	...
	public JSONObject UpdatePOForAcceptReject(String id_po,String status_approv,int id_emp_user,String dt_approv)
	{
		...
		JSONObject json=new JSONObject();
		 ...
		 query="update P_Purchase_Order set dt_approv3 ='"+dt_approv+"',st_approv3 ='"+status_approv+"',approv_by3="+id_emp_user+" where id_po="+id_po+"";
	...
	public JSONObject DisplayPurchaseOrderForApproval(String temp)
	{
		...
		if(!temp.equals(""))
		{
			sql = "select distinct(po.id_po),no_po,no_sap,(replace(convert(NVARCHAR, dt_po, 103), ' ', '-')) as dt_po,nm_emp,nm_ven,no_quot from P_Purchase_Order po,P_Purchase_Order_Asset poa,M_Emp_User emp,M_Vendor v "+
					" where po.id_po=poa.id_po and po.id_ven=v.id_ven and po.add_by=emp.id_emp_user and st_approv2='Accepted' and st_approv3='Waiting' and po.cancel=0 "+temp+"";
		}
		else
		{
			sql = "select distinct(po.id_po),no_po,no_sap,(replace(convert(NVARCHAR, dt_po, 103), ' ', '-')) as dt_po,nm_emp,nm_ven,no_quot from P_Purchase_Order po,P_Purchase_Order_Asset poa,M_Emp_User emp,M_Vendor v "+
					" where po.id_po=poa.id_po and po.id_ven=v.id_ven and po.add_by=emp.id_emp_user and st_approv2='Accepted' and st_approv3='Waiting' and po.cancel=0 ";
		}

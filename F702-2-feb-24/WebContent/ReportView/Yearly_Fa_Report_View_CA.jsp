...
<%
	try{
		...
		try{
		 ...
		 Group_id=request.getParameter("group_names");
		 fin_id=request.getParameter("fin_years");
		String location="";//request.getParameter("location");
		String actNameType="CASLM";
		String fin_years ="";
...
//out.print("Select std_finance,end_finance from M_Finance_Year "+ fin_years + " ORDER BY std_finance");
		rs5= sm5.executeQuery("Select std_finance,end_finance,(replace(convert(NVARCHAR, std_finance, 103), ' ', '-')) as std_finance1,(replace(convert(NVARCHAR, end_finance, 103), ' ', '-')) as end_finance from M_Finance_Year "+ fin_years + " ORDER BY std_finance");
		if(rs5.next()){
...
<tr>
	<td style="background-color: blue;" width="100%" class="title" colspan ="28"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 500px;">Companies Act Asset Report for  <b><%=Fin_stdate1 %> to <%=Fin_enddate1 %></b> Financial Year </p></td>
</tr>
<tr>
	...
	<td  nowrap rowspan="3" class="title" style="background-color: lavender;"><b>Invoice_Number</b></td>
...
<%
		...
		rs=sm.executeQuery("Select * from stline_depreciation where startdate='"+Fin_stdate+"' "+ grpCondn +" ");
		if(rs.next()){
...
<%
	try{
		 ...
		 */
		rs1=sm1.executeQuery("Select  sd.ast_id,sd.assetid,hr.bd as bond,hr.id_grp as group_id,hr.id_sgrp as subgroup_id,no_inv as invoicenumber,no_po as ponumber,hr.remarks as asset_remarks,(replace(convert(NVARCHAR, dt_allocate, 103), ' ', '-')) as instl_date,id_ven_proc as procured_vendor_id,ds_pro as assetname,qty_asst as assetqty,no_model as model_no,no_mfr as serialno,bd as assetbond,id_loc as loc_id,id_subl as subloc_id,id_flr as facility_id,to_assign as assignedto,id_cc as costcenter_id,allocate as installed,sd.asset_opening,sd.asset_addition,sd.asset_deletion,sd.asset_total,sd.asondate,sd.depreciated_opening,sd.depreciated_addition,sd.depreciated_deletion,sd.depreciated_total,sd.wdv,sd.writeoff,hr.id_sgrp as subgroup_id,hr.id_loc,hr.id_building from A_Ware_House hr,stline_depreciation sd where hr.id_wh_dyn=sd.assetid "+ grpCondn1 +
...
<%
	...
	rs2=sm2.executeQuery("Select nm_subl as subloc_name from M_Subloc where id_sloc='"+rs1.getInt("subloc_id")+"'");
	if(rs2.next())
		locationname=rs2.getString(1);
		//subloc=rs2.getString(1);
	
	rs2=sm2.executeQuery("Select nm_flr as facility_name from M_Floor where id_flr='"+rs1.getInt("facility_id")+"'");
	if(rs2.next()){

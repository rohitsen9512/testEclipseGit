...
<title>Transfer Receive Report</title>
...
<div id="PreviewGRNApprovePendingReport" class="watermarked" data-watermark="TRAIL..">
	     ...
	     DateFormat dateFormatNeeded = new SimpleDateFormat("yyyy-MM-dd");  
		        ...
		        if (exportToExcel != null
		            ...
		            response.setHeader("Content-Disposition", "inline; filename="
		                    + "Transfer Receive Report.jsp.xls");
						...
						if(!id_loc.equals(""))
						{
							if(!id_sloc.equals(""))
							{
								if(!id_building.equals(""))
								{
									if(!id_flr.equals(""))
									{
										tempSql =" and wh.id_loc="+id_loc+" and wh.id_subl="+id_sloc+" and wh.id_flr = "+id_flr+"  and wh.id_building = "+id_building+" ";
									}
									else
									{
					 ...
					 */
					 if(UserType.equals("Super")){
						...
						sql="select req_no_transfer as req_no,(replace(convert(NVARCHAR, req_dt_transfer, 103), ' ', '-')) as dt_req,wh.id_wh_dyn, wh.ds_pro,(replace(convert(NVARCHAR, ih.dt_start_tran, 103), ' ', '-')) as dt_start_tran,(replace(convert(VARCHAR, ih.dt_recv, 103), ' ', '-')) as dt_recv,l.nm_loc,  "+
								...
								" 	 bul2.id_building=ih.id_building_tran and fl2.id_flr=ih.id_flr_tran and  iut_approval=4"+
							 " " +tempSql +tempSql1+"";
					 }
					 else
					 {
	...
	<table id="TransferRecieveReport"
		class="table table-bordered table-hover TransferRecieveReport">
		<!-- <table style="border: 1px solid white;" border="1" width="140%" height="100%"> -->
		

				<!-- <tr >
					<td colspan="15" style="background-color: blue;" width="100%"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 500px;"><strong>Transfer Report</strong></p></td>
				</tr> -->	
				<thead><tr>
...
{
%>
<%-- <a href="TransferReceive_Report.jsp?exportToExcel=YES&startdate=<%=startdate1%>&enddate=<%=enddate1%>&id_building=<%=id_building%>&id_flr=<%=id_flr%>&id_div=<%=id_div%>&id_loc=<%=id_loc%>&id_sloc=<%=id_sloc%>&id_grp=<%=id_grp%>&id_sgrp=<%=id_sgrp%>">
...
getButtonsForListView('TransferRecieveReport','Transfer_Recieve_Report_List');

...
<div id="CancelledPOReportView">
			...
			try
			{
				...
				con=Common.GetConnection(request);
				 ...
				 if(!startdate.equals("") && !enddate.equals(""))
					{
					 tempSql = " and dt_grn >='"+ dateFormatNeeded.format(userDateFormat.parse(startdate))+"' and dt_grn <= '"+dateFormatNeeded.format(userDateFormat.parse(enddate))+"'";
					}
				 
				sql="select no_inv,no_po, (replace(convert(NVARCHAR, dt_inv, 103), ' ', '-')) as dt_inv , (replace(convert(NVARCHAR, dt_po, 103), ' ', '-')) as dt_po ,no_grn, (replace(convert(NVARCHAR, dt_grn, 103), ' ', '-')) as dt_grn ,qty,gr_tot,un_prc,nm_ven ,nm_model from g_grn g,G_Grn_Asset ga,M_Vendor ven  ,m_model mm where ga.id_model=mm.id_model and ven.id_ven=g.id_ven and g.id_grn=ga.id_grn "+tempSql+"" ;
		...
		<table border="1" width="100%" height="100%">
				<tr>
					<td colspan="11" style="background-color: blue;" width="100%"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 50%;">GRN Report</p></td>
				</tr>	
				<tr>
					...
					<td style="background-color: lavender;"><strong>PO Date</strong></td>
						<td style="background-color: lavender;"><strong>Invoice Number</strong></td>
					<td style="background-color: lavender;"><strong>Invoice Date</strong></td>
						<td style="background-color: lavender;"><strong>GRN Number</strong></td>
					...
					<td style="background-color: lavender;"><strong>Total Value</strong></td>
					</tr>	
			...
			<%
				...
				%>
					...
					<td><%=rs.getString("no_inv")%></td>
					<td><%=rs.getString("dt_inv")%></td>
					<td><%=rs.getString("no_grn")%></td>
					<td><%=rs.getString("dt_grn")%></td>
					
					
					<td><%=rs.getString("nm_ven")%></td>
					<td><%=rs.getString("nm_model")%></td>
				 	<td><%=rs.getString("qty")%></td>
					<td><%=rs.getString("gr_tot")%></td>
				 </tr>
				<%}
			}
			catch(Exception e)
...
{
%>
<a href="Grn_ReportView.jsp?exportToExcel=YES&enddate=<%=enddate%>&startdate=<%=startdate%>">

...
<title>Status Invoice Detail Report</title>
...
+ "Status_Invoice_Details_Report.xls");
...
{
	sql="select distinct led_m.lead_no,led_m.nm_pstn,ms.nm_subl ,(replace(convert(VARCHAR, led_m.dt_inv, 106), ' ', '-')) as invdt ,"
			+" led_m.inv_no from O_create_inv__msater_history led_m,O_create_invoice_history led ,M_Subloc ms where led_m.id_sloc=ms.id_sloc "
			+"and led_m.dt_inv between '"+std_finance+"' and '"+end_finance+"' and led_m.id_cr_inv_m_hist=led.id_cr_inv_m_hist  and inv_no IS NOT NULL ";
}
else{
	/* sql="select distinct led.* ,led_m.*,ms.nm_subl ,(replace(convert(VARCHAR, led_m.dt_tag, 106), ' ', '-')) as invdt from O_Assign_Lead_Master led_m,O_Assign_Lead led , M_Subloc ms,M_Loc l,O_Deliver_Detail_Lead del where " 
		      +"led_m.id_loc=l.id_loc and led_m.id_sloc=ms.id_sloc and led_m.id_lead_m=led.id_lead_m  and  l.id_loc=1 and  ms.id_sloc=1 and led_m.dt_tag >='2023-02-13' and led_m.dt_tag <='2023-03-13' order by dt_tag "; 

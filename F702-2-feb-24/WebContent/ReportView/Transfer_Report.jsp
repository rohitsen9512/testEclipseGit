...
<div id="PreviewGRNApprovePendingReport" class="watermarked" data-watermark="TRAIL..">
					 ...
					 */		
					 if(UserType.equals("Super")){		 
							sql="select req_no_transfer as req_no,(replace(convert(NVARCHAR, req_dt_transfer, 103), ' ', '-')) as dt_req,wh.id_wh_dyn, wh.ds_pro,(replace(convert(NVARCHAR, ih.dt_start_tran, 103), ' ', '-')) as dt_start_tran,(replace(convert(VARCHAR, ih.dt_recv, 103), ' ', '-')) as dt_recv,l.nm_loc,  "+
									...
									" 	 bul2.id_building=ih.id_building_tran and fl2.id_flr=ih.id_flr_tran and  (iut_approval>=3)"+
								 " " +tempSql +tempSql1+"";
					 }
					 else{
	...
	<section class="content-header">
		...
		class="table table-bordered table-hover TransferReport">
...
{
%>
<%-- <a href="Transfer_Report.jsp?exportToExcel=YES&startdate=<%=startdate1%>&enddate=<%=enddate1%>&id_building=<%=id_building%>&id_flr=<%=id_flr%>&id_div=<%=id_div%>&id_loc=<%=id_loc%>&id_sloc=<%=id_sloc%>&id_grp=<%=id_grp%>&id_sgrp=<%=id_sgrp%>">
...
getButtonsForListView('TransferReport','Transfer_Report_List');

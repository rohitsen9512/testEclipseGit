			...
			try
			{
				...
				DtoCommon dtoCommon = new DtoCommon();
					 ...
					 if(UserType.equals("Super")){
						...
						//	sql="select distinct led.* ,led_m.*,l.nm_loc,ms.nm_subl ,(replace(convert(VARCHAR, del.dt_install, 106), ' ', '-')) as dt_install, (replace(convert(VARCHAR, del.dt_install, 106), ' ', '-')) as dt_expinstall from O_Assign_Lead_Master led_m,O_Assign_Lead led , M_Subloc ms, M_Loc l,O_Deliver_Detail_Lead del where  led_m.state ='Closed' and led_m.tagto_me_state ='Closed' and led_m.id_loc=l.id_loc and led_m.id_sloc=ms.id_sloc and led_m.id_lead_m=led.id_lead_m  and  led.typ_service='sale'" ;
							/* sql="select distinct led.nm_prod,led.typ_service,led.quantity ,led.sr_no, led_m.nm_pstn,led_m.lead_no,led_m.adhar_no,ms.nm_subl ,"
									+" (replace(convert(VARCHAR, led_m.dt_ld, 106), ' ', '-')) as dt_install from O_Assign_Lead_Master led_m,"
									 +" O_Assign_Lead led ,M_Subloc ms, O_Deliver_Detail_Lead del where  (led_m.state ='Inprogress' and "
									+" led_m.tagto_me_state ='Inprogress' or  led_m.state ='Closed' and led_m.tagto_me_state ='Closed' ) and "
									+" led_m.status_deliver_ld=1 and   led_m.id_sloc=ms.id_sloc and led_m.id_lead_m=led.id_lead_m  and  led.typ_service='sale' and led_m.dt_ld between '"+std_finance+"' and '"+end_finance+"' ";  
							 */
							sql="select distinct led.nm_prod,led.typ_service,led.quantity ,led.sr_no, led_m.nm_pstn,led_m.lead_no,led_m.adhar_no,ms.nm_subl ,"
									+" (replace(convert(VARCHAR, led_m.dt_ld, 106), ' ', '-')) as dt_install from O_Assign_Lead_Master led_m,"
									 +" O_Assign_Lead led ,M_Subloc ms, O_Deliver_Detail_Lead del where  (led.state ='Inprogress' and "
									+" led.tagto_me_state ='Inprogress' or  led.state ='Closed' and led.tagto_me_state ='Closed' ) and "
									+" led_m.status_deliver_ld=1 and   led_m.id_sloc=ms.id_sloc and led_m.id_lead_m=led.id_lead_m  and  led.typ_service='sale' and led_m.dt_ld between '"+std_finance+"' and '"+end_finance+"' ";  
					 ...
					 else
					 {
						 ...
						 sql="select distinct led.nm_prod,led.typ_service,led.quantity ,led.sr_no,led_m.nm_pstn,led_m.lead_no,led_m.adhar_no,ms.nm_subl ,"
									+" (replace(convert(VARCHAR, led_m.dt_ld,  106), ' ', '-')) as dt_install from O_Assign_Lead_Master led_m,"
									 +" O_Assign_Lead led ,M_Subloc ms, M_Loc l,O_Deliver_Detail_Lead del where  (led_m.state ='Inprogress' and "
									+" led_m.tagto_me_state ='Inprogress' or  led_m.state ='Closed' and led_m.tagto_me_state ='Closed' ) and "
									+" led_m.status_deliver_ld=1 and  led_m.id_sloc=ms.id_sloc and led_m.id_lead_m=led.id_lead_m  and  led.typ_service='sale' and led_m.dt_ld between '"+std_finance+"' and '"+end_finance+"' ";  
					 }
	...
	<table id="StatutusRenatalProdReport"
		...
		<!-- <table style="border: 1px solid white;" border="1" width="100%" height="100%">
			...
			<th><strong><center>Lead No. </center></strong></th>
			 ...
			 <th><strong><center>Serial Name</center></strong></th>

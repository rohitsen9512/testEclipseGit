...
<title>Short Close PO Report</title>
...
<div id="CancelledPOReportView">
		...
		String Group_id="",fin_id="",curr="";
			...
			try
			{
				...
				sql="Select nm_ven,p.no_po,(replace(convert(NVARCHAR, p.dt_short_close, 103), ' ', '-')) as dt_short_close,(replace(convert(NVARCHAR, p.dt_po, 103), ' ', '-')) as dt_po,p.tot,p.rem_val_po,p.typ_pmt,p.no_quot "+
						" from P_Purchase_Order p,m_vendor ven where ven.id_ven=p.id_ven and shortclosed=1  "+tempSql+"" ;
		...
		<table border="1" width="100%" height="100%">
				<tr>
					<td colspan="9" style="background-color: blue;" width="100%"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 50%;">Short Close PO Report</p></td>
				</tr>	
				<tr>
					...
					<td style="background-color: lavender;"><strong>Short Close Date</strong></td>
					...
					<td style="background-color: lavender;"><strong>Payment Type</strong></td>
					<td style="background-color: lavender;"><strong>PO Value</strong></td>
					<td style="background-color: lavender;"><strong>Remaining PO Value</strong></td>
				</tr>	
			...
			<%
				...
				%>
					...
					<td><%=rs.getString("dt_po")%></td>
						<td><%=rs.getString("dt_short_close")%></td>
					<td><%=rs.getString("nm_ven")%></td>
					<td><%=rs.getString("no_quot")%></td>
					<td><%=rs.getString("typ_pmt")%></td>
					<td><%=rs.getString("tot")%></td>
					<td><%=rs.getString("rem_val_po")%></td>
				</tr>
				<%}
			}
			catch(Exception e)
...
{
%>
<a href="ShortClosePO_ReportView.jsp?exportToExcel=YES&enddate=<%=enddate%>&startdate=<%=startdate%>">

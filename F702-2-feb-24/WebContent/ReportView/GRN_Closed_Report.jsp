...
<div id="PreviewGRNClosedReport">
		...
		String Group_id="",fin_id="";
			...
			try
			{
				...
				sql="select distinct(id_wh_dyn), g.no_grn, wh.ds_pro, wh.mfr, (replace(convert(NVARCHAR, g.dt_grn, 103), ' ', '-')) as dt_grn,g.no_po,(replace(convert(NVARCHAR, g.dt_po, 103), ' ', '-')) as dt_po,g.no_inv,(replace(convert(NVARCHAR, g.dt_inv, 103), ' ', '-')) as dt_inv, "+
						" g.no_dc,(replace(convert(NVARCHAR, g.dt_dc, 103), ' ', '-')) as dt_dc ,l.nm_loc, sl.nm_subl,v.nm_ven,g.remarks,inm.bd,inm.no_boe,g.no_aprv_sez,qty,qty_recv from A_Ware_House wh, M_Loc l,M_Subloc sl,G_Grn g, A_Invoice_Master inm, M_Vendor v,A_Invoice inv "+
						...
						" and inv.id_inv_m=inm.id_inv_m and g.dt_grn >='"+startdate+"' and g.dt_grn <= '"+enddate+"'";
							...
							sql="select distinct(id_wh_dyn), g.no_grn, wh.ds_pro, wh.mfr, (replace(convert(NVARCHAR, g.dt_grn, 103), ' ', '-')) as dt_grn,g.no_po,(replace(convert(NVARCHAR, g.dt_po, 103), ' ', '-')) as dt_po,g.no_inv,(replace(convert(NVARCHAR, g.dt_inv, 103), ' ', '-')) as dt_inv, "+
									" g.no_dc,(replace(convert(NVARCHAR, g.dt_dc, 103), ' ', '-')) as dt_dc ,l.nm_loc, sl.nm_subl,v.nm_ven,g.remarks,inm.bd,inm.no_boe,g.no_aprv_sez,qty,qty_recv from A_Ware_House wh, M_Loc l,M_Subloc sl,G_Grn g, A_Invoice_Master inm, M_Vendor v,A_Invoice inv "+
									" where l.id_loc=inm.id_loc and sl.id_sloc=inm.id_subl and wh.id_grn=g.id_grn and v.id_ven=inm.id_ven and g.id_inv_m=inm.id_inv_m and inv.typ_asst = '"+UserType+"' "+
									" and inv.id_inv_m=inm.id_inv_m and g.dt_grn >='"+startdate+"' and g.dt_grn <= '"+enddate+"'";
			...
			%>
		<table border="1" width="170%" height="100%">
		

				<tr >
					<td colspan="21" style="background-color: blue;" width="100%"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 500px;">GRN Done</p></td>
				</tr>	
				<tr>
					...
					<td style="background-color: lavender;"><strong>Total Qty&nbsp;&nbsp;</strong></td>
					<td style="background-color: lavender;"><strong>Receive Qty&nbsp;&nbsp;</strong></td>
...
{
%>
<a href="GRN_Closed_Report.jsp?exportToExcel=YES&enddate=<%=enddate1%>&startdate=<%=startdate1%>">

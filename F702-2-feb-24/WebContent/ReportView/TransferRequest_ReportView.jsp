...
<title>Transfer Request Report</title>
...
<div id="PreviewGRNApprovePendingReport" class="watermarked" data-watermark="TRAIL..">
	     ...
	     DateFormat dateFormatNeeded = new SimpleDateFormat("yyyy-MM-dd");  
			...
			try
			{
				...
				sql="select req_no_transfer as req_no,(replace(convert(NVARCHAR, req_dt_transfer, 103), ' ', '-')) as dt_req, wh.id_wh_dyn, wh.ds_pro,(replace(convert(NVARCHAR, ih.dt_start_tran, 103), ' ', '-')) as dt_start_tran,(replace(convert(VARCHAR, ih.dt_recv, 103), ' ', '-')) as dt_recv,l.nm_loc,  "+
						...
						" 	 bul2.id_building=ih.id_building_tran and fl2.id_flr=ih.id_flr_tran  "+
					 " " +tempSql +tempSql1+"";
					 }
					 else{
	...
	<table id="TransferRequestReport"
		class="table table-bordered table-hover TransferRequestReport">
		<!-- <table style="border: 1px solid white;" border="1" width="140%" height="100%">
		 -->

				<!-- <tr >
					<td colspan="15" style="background-color: blue;" width="100%"><p  style="background-color: blue; font-size: 24px;color: white;margin-left: 500px;"><strong>Transfer Request Report</strong></p></td>
				</tr> -->	
			<thead>	<tr>
...
{
%>
<%-- <a href="TransferRequest_ReportView.jsp?exportToExcel=YES&startdate=<%=startdate1%>&enddate=<%=enddate1%>&id_building=<%=id_building%>&id_flr=<%=id_flr%>&id_div=<%=id_div%>&id_loc=<%=id_loc%>&id_sloc=<%=id_sloc%>&id_grp=<%=id_grp%>&id_sgrp=<%=id_sgrp%>">
...
getButtonsForListView('TransferRequestReport','Transfer_Request_Report_List');

	...
	if(action=='Cancel')
		{
			...
			$('#'+displayContent).show();
			DisplayPurchaseOrderForInvoice(servletName,displayContent,createDetails,'','invoiceForDisplay');
		}
				...
				if(r.data)
					{
						...
						$('input[name="nm_acc_asset"]').val(r.data[0].ds_pro);
...
$.post('Invoice_Through_PO',{action : 'EditAsset',id_po_asst : id_po_asst},function (r){
		...
		if(r.data)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><strong><a class="alert" href="#" onclick="CreateAccessoryForPO(\'Modify\',\'displayAccessory\',\'createAccessory\','+params.id_po_asst+','+params.id_grp+')"> Create Invoice </a></strong></td>'+
									'</tr>';
				}
...
function DisplayPurchaseOrderForInvoice(servletName,displayContent,createDetails,no_inv,DisplayTableClass)
{
SessionCheck(function (ses){
		...
		if(ses)
			...
			var dt_to =$('#dt_to').val();
			var search = $('#SearchFromForPOValue').val();
				$.post(servletName,{action : 'Display' , no_inv : no_inv,dt_frm:dt_frm,dt_to:dt_to, search:search},function (r){
					
					var list= '<tr class="tableHeader info">'+
					/*'<td><strong>Quotation Number</strong></td>'+
					'<td><strong>Quotation Date</strong></td>'+
					'<td><strong>SAP Number</strong></td>'+*/

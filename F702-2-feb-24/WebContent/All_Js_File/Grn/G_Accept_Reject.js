		...
		$('.invoiceByPOBtnForDissable').attr('disabled','disabled');
			$.post(servletName,x,function (r){
				...
				if(r.data1 == 1)
					{
					$('#createAccessory').hide();
					bootbox.alert('GRN Creation successfully');
					 DisplayinvoiceforGRN('G_Accept_Reject','displayInvoice','createInvoice','','invoiceForDisplay');
...
function DisplayinvoiceforInvoiceForAcceptReject(servletName,displayContent,createDetails,no_inv,DisplayTableClass)
{
SessionCheck(function (ses){
		...
		if(ses)
			...
			var searchWord = $('input[name="search"').val();
				$.post(servletName,{action : 'DisplayForApprove' , no_inv : no_inv,dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){
				...
				if(r.data.length > 0)
					{
						...
						'<td><strong><center><a class="alert" href="#" onclick="BeforeEditAssetPOForApproveReject(\''+servletName+'\',\''+displayContent+'\',\''+createDetails+'\','+params.id_inv_m+')"> Preview </a></center></strong></td>'+
											'</tr>';
						}
...
function DisplayinvoiceforGRN(servletName,displayContent,createDetails,no_inv,DisplayTableClass)
{
SessionCheck(function (ses){
		...
		if(ses)
			...
			var searchWord = $('input[name="search"').val();
				$.post(servletName,{action : 'Display' , no_inv : no_inv,dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){
					...
					'<td><strong><center>Vendor</center></strong></td>'+
					'<td style="width: 170px;"><strong><center><a href="#">Create GRN</a></center></strong></td>'+
				'</tr>';
				...
				if(r.data.length > 0)
					{
						...
						'<td><center>'+params.nm_ven+'</center></td>'+
						'<td><strong><center><a class="alert" href="#" onclick="BeforeEditAssetPO(\''+servletName+'\',\''+displayContent+'\',\''+createDetails+'\','+params.id_inv_m+')"> Create GRN </a></center></strong></td>'+
											'</tr>';
						}
...
function BeforeEditAssetPOForApproveReject(servletName,displayContent,createDetails,id,id_dept,id_cc,id_section,po_type){
	
	EditAssetPOApproveReject(servletName,displayContent,createDetails,id,po_type);
}

function EditAssetPOApproveReject(servletName,displayContent,createDetails,id,po_type)
{
SessionCheck(function (ses){
	...
	var t=0;
		$.post(servletName,{action : 'Edit',id : id},function (r){
				...
				if(t == 1)
				{
					...
					DisplayAccessoryForPOApproveReject('G_Accept_Reject','displayInvoice','createInvoice',id,'accessoryForDisplay');
...
function DisplayAccessoryForPOApproveReject(servletName,displayContent,createDetails,id,DisplayTableClass)
{
SessionCheck(function (ses){

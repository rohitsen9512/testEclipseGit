...
function ControlViewQuotationDiv(action , DisplayDiv , HideDiv , no_ind)
{
...
function EditViewQuotaionFun(action , DisplayDiv , HideDiv , no_ind)
{
	SessionCheck(function (ses){		
		if(ses)
			{
		$.post('Approval_Quotation',{action : 'ViewEdit' , no_ind : no_ind},function (r){
			var list='', temp='';
...
function DisplayQuotaionForPreview()
{
	SessionCheck(function (ses){		
		if(ses)
			{
	...
	var searchWord = $('input[name="search"').val();
		$.post('Approval_Quotation',{action : 'ViewDisplay',dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><strong><center><a class="alert" href="#" onclick="ControlViewQuotationDiv(\'Modify\',\'createPreviewQuotation\',\'displayPreviewQuotation\',\''+params.no_ind+'\')"> Preview </a></center></strong></td>'+
									'</tr>';
				}

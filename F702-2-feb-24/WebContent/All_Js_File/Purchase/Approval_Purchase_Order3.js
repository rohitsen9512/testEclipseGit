function ControlApprovalPurchaseOrder3(action,DisplayDiv,HideDiv,id_po)
{
	...
	if(action == 'Modify')
	{
		PurchaseOrderForApprove3(action,DisplayDiv,HideDiv,id_po);
	}
	if(action =='Accepted' || action =='Rejected')
	{
		...
		if(dt_approv != '')
		{
		$.post('Approval_Purchase_Order3',{action:'CheckDate',id_po:id_po,dt_approv:dt_approv},function (r){
		if(r.data == 1)
		{
			$('.poApprv3').attr('disabled','disabled');
			$.post('Approval_Purchase_Order3',{action:'Update',status_approv:action,id_po:id_po,dt_approv:dt_approv},function (r){
				if(r.data == 1)
					{
						bootbox.alert("Updated Successfully.");
						$( ".approvePurchaseOrder3" ).trigger( "click" );
					}
				else
				{
					bootbox.alert("Something went wrong Please try again.");
				}
				$('.poApprv3').removeAttr('disabled');	
			},'json');
...
function PurchaseOrderForApprove3(action,DisplayDiv,HideDiv,id_po)
{
	$.post('Approval_Purchase_Order3',{action : 'Edit',id_po:id_po},function (r){
		if(r.data.length > 0)
			{
	    ...
	    '</tr>'+
		 ...
		 '<tr>'+
         '	<td colspan="8">'+
         '	<button type="button" style="margin-left: 400px;" class="btn btn-primary poApprv3" onclick="ControlApprovalPurchaseOrder3(\'Accepted\',\'displayApprovePurchaseOrder3\',\'createApprovePurchaseOrder3\','+id_po+')">Approve </button>&nbsp;&nbsp;'+
         '	<button type="button" class="btn btn-primary poApprv3" onclick="ControlApprovalPurchaseOrder3(\'Rejected\',\'displayApprovePurchaseOrder3\',\'createApprovePurchaseOrder3\','+id_po+')">Reject </button>&nbsp;&nbsp;'+
         '	<button type="button" class="btn btn-primary poApprv3" onclick="ControlApprovalPurchaseOrder3(\'Cancel\',\'displayApprovePurchaseOrder3\',\'createApprovePurchaseOrder3\',\'\')">Cancel </button>'+
		 '	</td>'+
		'</tr>';
			
				
			$('.PurchaseOrderDetailsForApproval3').html(list);
...
function DisplayPurchaseOrderForApproval3()
{
	...
	var typ_po =$('select[name="typ_po"]').val();
		...
		$.post('Approval_Purchase_Order3',{action : 'Display',dt_frm:dt_frm,dt_to:dt_to,typ_po:typ_po},function (r){
			...
			'<tr class="tableHeader info">'+
				...
				'<td><center><strong><center>SAP Number</strong></center></td>'+
				'<td><center><strong><center>Quotation Number</strong></center></td>'+
				'<td><center><strong><center>Vendor Name </strong></center></td>'+
				'<td><center><strong><center>Raised By</strong></center></td>'+
				'<td><center><strong><center>Raised Date</strong></center></td>'+
				'<td style="width: 70px;"><strong><center><center><a href="#">Preview</a></center></strong></td>'+
		'</tr>';
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><strong><center><a href="#" onclick="ControlApprovalPurchaseOrder3(\'Modify\',\'createApprovePurchaseOrder3\',\'displayApprovePurchaseOrder3\',\''+params.id_po+'\')"> Preview </a></center></strong></td>'+
									'</tr>';
				}
			
				$('.DisplayPurchaseOrderForApproval3').html(list);
			}

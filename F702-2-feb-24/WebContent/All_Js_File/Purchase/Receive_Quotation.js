function ControlReceiveQuotation(action ,DispDiv , HideDiv , no_ind)
{
	...
	if(action == 'Save')
	{
		...
		if(t)
			{bootbox.confirm("Are you sure ?", function(result) {
				...
				$.post('Receive_Quotation',x,function (r)
						{
	...
	if(action == 'Update')
	{
		...
		var t=false;
		t=ValidationForm('updateReceiveQuotation');
		if(t)
		{
			$('.upd').attr('disabled','disabled');

			
			var x = $('#updateReceiveQuotation').serialize();
			console.log(x);
			$.post('Receive_Quotation',x,function (r){
				if(r.data == 1)
					{
...
function DisplayQuotationDetails(DispDiv , HideDiv , no_ind ,id_ven,nm_ven,id_req_quot,id_req_quot_ven,msme,nsic,id_curr)
{
	
	
	$.post('Receive_Quotation',{action : 'DisplayQuotationDetails',no_ind:no_ind},function (r){
		...
		/*'<td style="width: 100px;"><strong><center>Delivery Date <font color="red"> * </font></center></strong></td>'+
		*/
		...
		if(r.data.length > 0)
			...
			var grnd_tot=0.0;
				...
				list += '<tr class="success">'+
					...
					'<td><center><input style="width: 135px;" type="text" name="remarks'+i+'" value="" class="common-validation" ></center></td>'+
					...
					'<td><center><input type="text" name="un_prc'+i+'"  value="0" patelUnPrc="'+i+'"  style="width: 60px;text-align: right;" onChange="calculateTot(event,\'un_prc\')" class="patelUnPrc common-validation" data-valid="num" ></center></td>'+
					'<td><input type="text" name="others'+i+'"  value="0.0" patelUnPrc="'+i+'" style="float:right;text-align: right;width:60px""  onChange="calculateTot(event,\'others\')" class="common-validation" ></td>'+
					'<td><center>'+
					...
					'</select>'+
					'</center></td>'+
					'<td><center><input type="text"  name="tax_val1'+i+'"  value="0.0" patelUnPrc="'+i+'" onChange="calculateTot(event,\'val_tax1\')" style="width:60px;text-align: right;" class="common-validation" readonly ></center></td>'+
					'<td><center><input type="text"  name="tax_val2'+i+'"  value="0.0" patelUnPrc="'+i+'" onChange="calculateTot(event,\'val_tax2\')" style="width:60px;text-align: right;" class="common-validation" readonly></center></td>'+
					'<td><center><input type="text"  name="buyback'+i+'"  value="0.0" patelUnPrc="'+i+'"  onChange="calculateTot(event,\'buyback\')" style="width:60px;text-align: right;" class="common-validation" ></center></td>'+
					
					'<td><center><input type="text"  name="gr_tot'+i+'"  value="'+grnd_tot+'" style="width:60px;text-align: right;" class="common-validation commonTotal" readonly></center></td>'+
				...
				list +='<tr>'+
						'<td colspan="11"><input type="text" name="oter_text" value="Others Charge" style="float:right;"  class="common-validation" ></td>'+
							...
							'<td><center><input type="text" name="frt_text" value="0.00" style="width: 60px;text-align: right" data-valid="num" onChange="addInTotal(\'frt_text\')" class="common-validation" ></center></td>'+
						'</tr>'+
						
						'<tr>'+
						'<td colspan="11"><b style="float:right;">Discount</b></td>'+
						'<td><center><input type="text" name="discount" value="0.00" style="width: 60px;text-align: right" data-valid="num" onChange="addInTotal(\'discount\')" class="common-validation" ></center></td>'+
					'</tr>'+
						'<tr>'+
							'<td colspan="11"><b style="float:right;">Grand Total</b></td>'+
							'<td><center><input type="text" name="tot" value="'+grnd_tot+'" style="width: 60px;text-align: right" class="common-validation" readonly></center></td>'+
					'</tr>'+
				
				
			 '<tr>'+
	         '	<td colspan="12">'+
	         '	<button type="button" style="margin-left: 400px;" class="btn btn-primary rcvQuotBtnForDissable" onclick="ControlReceiveQuotation(\'Save\',\'displayReceiveQuotation\',\'createReceiveQuotation\',\'\')">Save </button>&nbsp;&nbsp;'+
			 ...
			 '	</td>'+
			'</tr>';
				$('input[name="id_req_quot"]').val(id_req_quot);
				$('input[name="id_ven"]').val(id_ven);
				$('input[name="id_req_quot_ven"]').val(id_req_quot_ven);
				$('input[name="nm_ven"]').val(nm_ven);
...
$( ".recvQuotDatepicker2" ).datepicker({
	yearRange: '1985:2025',
      ...
      onSelect: function(selected,evnt) {
    	  ...
    	  $.post('Receive_Quotation',{action : 'CheckDate' ,dt_recv:dt_recv,id_req_quot:id_req_quot},function (r){
	    		if(r.data == 0)
	    			{
...
function DisplayReceiveQuotation(servletName,displayContent,createDetails)
{
	...
	$.post(servletName,{action : 'Display',dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){
		...
		var list= '<tr>'+
		'<td colspan="5" class="tableHeader"><p class="tableHeaderContent" style="margin-left: 400px;">List Of RFQ</p></td>'+
		'</tr>'+
			'<tr class="tableHeader info">'+
			'<td><center><strong>Vendor Name</strong></center></td>'+
			'<td><center><strong>Consolidated PI No</strong></center></td>'+
			'<td><center><strong>Quot Requested Date</strong></center></td>'+
			'<td><center><strong>Requested By</strong></center></td>'+
		'<td style="width:200px;"><center><a href="#"><strong>Submit RFQ</strong></a></center></td>'+
	'</tr>';
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><center><a class="alert" href="#" onclick="DisplayQuotationDetails(\'createReceiveQuotation\',\'displayReceiveQuotation\',\''+params.no_ind+'\','+params.id_ven+',\''+params.nm_ven+'\','+params.id_req_quot+','+params.id_req_quot_ven+',\''+params.msme+'\',\''+params.nsic+'\','+params.curr+')">Submit RFQ</a></center></td>'+
							  '</tr>';
				}
...
function DisplayModifyReceiveQuotation()
{
	...
	var dt_to =$('input[name="dt_to"]').val();
	var searchWord = $('input[name="search"').val();
		$.post('Receive_Quotation',{action : 'Modifyrecvquotation',searchWord:searchWord},function (r){
...
function ControlmodifyReceiveQuotation(action,DisplayDiv,HideDiv,no_quot,no_ind,id_quot)
{
	...
	$.post('Receive_Quotation',{action : 'Modifyrecvquotation',no_ind:no_ind,id:id_quot},function (r){
		if(r.data.length > 0)
			{
				for(var i=0;i<r.data.length;i++)
				{

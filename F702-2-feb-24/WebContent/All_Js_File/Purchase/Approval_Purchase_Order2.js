function ControlApprovalPurchaseOrder2(action,DisplayDiv,HideDiv,id_po)
{
	...
	if(action == 'Modify')
	{
		PurchaseOrderForApprove2(action,DisplayDiv,HideDiv,id_po);
	}
	if(action =='Accepted' || action =='Rejected')
	{
		...
		if(dt_approv != '')
		{
			console.log(dt_approv);
		$.post('Approval_Purchase_Order2',{action:'CheckDate',id_po:id_po,dt_approv:dt_approv},function (r){
		if(r.data == 1)
		{
			$('.poApprv2').attr('disabled','disabled');
			$.post('Approval_Purchase_Order2',{action:'Update',status_approv:action,id_po:id_po,dt_approv:dt_approv},function (r){
				if(r.data == 1)
					{
						bootbox.alert("Updated Successfully.");
						$( ".approvePurchaseOrder2" ).trigger( "click" );
					}
				else
				{
					bootbox.alert("Something went wrong Please try again.");
				}
				$('.poApprv2').removeAttr('disabled');
			},'json');
					
		}
		else
			{
			console.log(r.dt_approv);
...
function PurchaseOrderForApprove2(action,DisplayDiv,HideDiv,id_po)
{
	$.post('Approval_Purchase_Order2',{action : 'Edit',id_po:id_po},function (r){
		if(r.data.length > 0)
			{
		...
		'</tr>'+*/
		'<tr>'+
			'<td style="border-top-style: hidden;"><b>Approver Name : </b></td>'+
		...
		'</tr>'+
		'<tr>'+
			'<td colspan="7" style="border-top-style: hidden;"><b style="float:right;">Frieght </b></td>'+
	    ...
	    '</tr>'+
		 ...
		 '<tr>'+
         '	<td colspan="8">'+
         '	<button type="button" style="margin-left: 400px;" class="btn btn-primary poApprv2" onclick="ControlApprovalPurchaseOrder2(\'Accepted\',\'displayApprovePurchaseOrder2\',\'createApprovePurchaseOrder2\','+id_po+')">Approve </button>&nbsp;&nbsp;'+
         '	<button type="button" class="btn btn-primary poApprv2" onclick="ControlApprovalPurchaseOrder2(\'Rejected\',\'displayApprovePurchaseOrder2\',\'createApprovePurchaseOrder2\','+id_po+')">Reject </button>&nbsp;&nbsp;'+
         '	<button type="button" class="btn btn-primary poApprv2" onclick="ControlApprovalPurchaseOrder2(\'Cancel\',\'displayApprovePurchaseOrder2\',\'createApprovePurchaseOrder2\',\'\')">Cancel </button>'+
		 '	</td>'+
		'</tr>';
...
function DisplayPurchaseOrderForApproval2()
{
	...
	var typ_po =$('select[name="typ_po"]').val();
		...
		$.post('Approval_Purchase_Order2',{action : 'Display',dt_frm:dt_frm,dt_to:dt_to,typ_po:typ_po},function (r){
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><strong><center><a href="#" onclick="ControlApprovalPurchaseOrder2(\'Modify\',\'createApprovePurchaseOrder2\',\'displayApprovePurchaseOrder2\',\''+params.id_po+'\')"> Preview </a></center></strong></td>'+
									'</tr>';
				}
			
				$('.DisplayPurchaseOrderForApproval2').html(list);
			}

...
function ControlEvaluate(action , DisplayDiv , HideDiv , no_rfi)
{
	...
	{
		EditFinalizeVendorFunsplit(action , DisplayDiv , HideDiv , no_rfi);
 						...
 						if(r.data == 1)
 							{
 								...
 								bootbox.alert("Partner has successfully Finilized.");
			 ...
			 var place=0;   var pr = 0;zx=1,qt=0; var no_ven=0;
				if(r.data.length > 0)
					...
					for(var j = 0; j < id_quotArr.length ; j++)
					{
						...
						'<td style="min-width:200px;"><strong><center>Item Name </strong></center></td>'+
				 ...
				 venSelectionList='<tr ><td colspan="4"></td></tr>'+
					 '<tr class="tableHeader info">'+
									...
									'<td style="width: 70px"><strong>Item Code</strong></td>'+
				...
				*/	'</td>'+
					...
					'</tr>'+	
							'<tr>'+
							'<td colspan="11">'+
								'<center><button type="button" style="margin-left: 200px;" class="btn btn-primary appq" onclick="ControlEvaluate(\'Accepted\',\'displayApproveQuotation\',\'createApproveQuotation\')">Send For Financial Approval</button></center>'+
...
function DisplayEvaluate()
{
	...
	var searchWord = $('input[name="search"').val();
		$.post('P_Create_RFI',{action : 'DisplayEvaluate',dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){

			var list= '<tr>'+
			'<td colspan="4" class="tableHeader"><p class="tableHeaderContent" style="margin-left: 400px;">RFI Details</p></td>'+
			'</tr>'+
				'<tr class="tableHeader info">'+
			'<td><strong>RFI No.<a href=#></a></strong></td>'+
			'<td><strong>RFI Date</strong></td>'+
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<a class="alert" href="#" onclick="ControlEvaluate(\'split\',\'createApproveQuotation\',\'displayApproveQuotation\',\''+params.no_rfi+'\')"> Evaluate RFI </a></strong></center>'+
...
function EditFinalizeVendorFunsplit(action , DisplayDiv , HideDiv , no_rfi)
{

	SessionCheck(function (ses){		
		if(ses)
			{
		$.post('P_reate_RFI',{action : 'EditEvaluate' , no_rfi : no_rfi},function (r){
			var list='', temp='';
			 var prit=0;
				if(r.data.length > 0)
					{
				 ...
				 '<td style="min-width:200px;"><strong><center>Item Name </strong></center></td>'+
					'<td><center><strong>Item Code</strong></center></td>'+
						 ...
						 '<input type="hidden" name="no_ind" value="'+no_ind+'">'+
							...
							'<option value=""> Select</option><option value="Item_wise"> Item Wise </option><option value="tot_wise"> Total Wise </option></select>'+
					...
					'<tr>'+
							'<td colspan="10">'+
								'<center><button type="button" style="margin-left: 200px;" class="btn btn-primary appq" onclick="ControlEvaluate(\'Rejected\',\'displayApproveQuotation\',\'createApproveQuotation\')">Save</button></center>'+
//								'<button type="button" class="btn btn-primary appq" style="margin-left: 400px;" onclick="ControlAccepRejectQuotationDiv(\'Rejected\',\'displayApproveQuotation\',\'createApproveQuotation\')">Reject</button></td>'+

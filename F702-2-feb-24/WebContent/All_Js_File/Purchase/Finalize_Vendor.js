...
function ControlFinalizeVendorDiv(action , DisplayDiv , HideDiv , no_ind)
{
		...
		if(result)
		{
 			...
 			$.post('Vendor_Acceptance',x,function (r){
 						if(r.data == 1)
 							{
 								DisplayFinalizeVendor();
 								bootbox.alert("Vendor has successfully Finilized.");
			 ...
			 var place=0;   var pr = 0;zx=1,qt=0; var no_ven=0;
				...
				*/	'</td>'+
					...
					'</tr>'+	
							'<tr>'+
							'<td colspan="11">'+
								'<center><button type="button" style="margin-left: 200px;" class="btn btn-primary appq" onclick="ControlFinalizeVendorDiv(\'Accepted\',\'displayApproveQuotation\',\'createApproveQuotation\')">Send For Financial Approval</button></center>'+
...
function DisplayFinalizeVendor()
{
	SessionCheck(function (ses){		
		if(ses)
			{
	...
	var searchWord = $('input[name="search"').val();
		$.post('Finalize_Vendor',{action : 'Display',dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){

			var list= '<tr>'+
			'<td colspan="4" class="tableHeader"><p class="tableHeaderContent" style="margin-left: 400px;">Pending Model For Vendor selection</p></td>'+
			'</tr>'+
				'<tr class="tableHeader info">'+
			'<td><strong>RFQ Number<a href=#></a></strong></td>'+
			'<td><strong>RFQ Date</strong></td>'+
			'<td><strong>RFQ By</strong></td>'+
			'<td style="width: 300px;"><center><strong><a href="#">Finalize Vendor</a></strong> </center></td>'+
		'</tr>';
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<a class="alert" href="#" onclick="ControlFinalizeVendorDiv(\'split\',\'createApproveQuotation\',\'displayApproveQuotation\',\''+params.no_ind+'\')"> Finalize Vendor </a></strong></center>'+
				...
				if(r.data.length > 0)
					...
					for(var j = 0; j < id_quotArr.length ; j++)
					{
						...
						for(var j = 0; j < id_quotArr.length ; j++)
						{
							...
							if(params.reject_ven=='1'){
								rejdisable.push(params.id_quot)
							}
							...
							'<td colspan="2">'+
							'<button type="button" style="float:right;margin-left:10px" class="btn btn-primary appq" id="rej'+params.id_quot+'" onclick="RejectVendor(\'Reject\','+params.id_ven+',\''+params.id_quot+'\',\''+params.no_quot+'\')">Reject</button></center>'+
					   ...
					   '<td ><strong><center>Quantity<a href=#></a></center></strong></td>'+
						...
						'</tr>';
							 ...
							 if(j==0){
				 venSelectionList='<tr class="tableHeader info">'+
				 ...
				 '<td style="min-width:200px;"><strong><center>Model Name </strong></center></td>'+
					...
					'<tr>'+
							'<td colspan="10">'+
								'<center><button type="button" style="margin-left: 200px;" class="btn btn-primary appq" onclick="ControlFinalizeVendorDiv(\'Rejected\',\'displayApproveQuotation\',\'createApproveQuotation\')">Save</button></center>'+
//								'<button type="button" class="btn btn-primary appq" style="margin-left: 400px;" onclick="ControlAccepRejectQuotationDiv(\'Rejected\',\'displayApproveQuotation\',\'createApproveQuotation\')">Reject</button></td>'+
						...
						for(var l = 0; l < negodisable.length ; l++)
						{
							...
							for(var l = 0; l < rejdisable.length ; l++)
						{
							$('#rej'+rejdisable[l]+'').attr('disabled','disabled');
							$('#nego'+rejdisable[l]+'').attr('disabled','disabled');
...
function Negotiation(action,id_ven,id_quot,no_quot){
	...
	function RejectVendor(action,id_ven,id_quot,no_quot){
		$('#rej'+id_quot+'').attr('disabled','disabled');
		
		 $.post('Finalize_Vendor',{action : 'RejectVendor', id_ven:id_ven, id_quot:id_quot,no_quot:no_quot},function (r){
			
				
					if(r.data=='1'){
						bootbox.alert("Vendor has been rejected.");
						$('#rej'+id_quot+'').attr('disabled','disabled');
				      $('#nego'+id_quot+'').attr('disabled','disabled');

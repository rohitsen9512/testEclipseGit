...
if(action == 'Cancel')
	...
	$.post('Vendor_Selection',x,function (r)
		{
				
			if(r.data == 1)
				{
					bootbox.alert("Successfully Added.");
					$( ".vendorSelection" ).trigger("click");
				}
				else
				{
...
function PreviewVendorSelection(DisplayDiv,HideDiv,no_quot,no_ind,id_quot)
{
	...
	console.log(valtech);
						...
						'</tr>';
							...
							'</tr>'+
								'<tr>'+
									'<td colspan="4"><strong>Did you competitively bid the order with multiple vendors ? </strong></td>'+
									'<td colspan="4" style="border-left-style: hidden;">'+
									'<input type="radio"  name="bidding" value="1"'+checked+'>&nbsp;Yes&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
									'<input type="radio"  name="bidding" value="0" '+checked1+'>&nbsp;No'+
									
									'</td> <br>'+
								'</tr>'+
								'<tr><td colspan="8" style="border-top-style: hidden;">Bids are required for amounts greater than $1000 / INR50,000</td></tr>'+
								'<tr><td colspan="8" style="border-top-style: hidden;">'+
								'<b>A. If Yes,</b><br>'+
								'<div align="center"><b>(i) List of bids received</b></div>'+
								'</td></tr>'+
							 '<tr>'+
							...
							'</tr>'+
							'<tr>'+
							'<td colspan="8" style="border-top-style: hidden;"><font size="2"><b>B. If no, state reason (check all that apply)</b></font></td>'+
							'</tr>'+
							'<tr>'+
								'<td colspan="8" style="border-top-style: hidden;">'+
							...
							'<tr>'+
								'<td colspan="8" style="border-top-style: hidden;">'+
									...
									'<textarea rows="2" name="other_specify" cols="40"  class="textarea" onKeyPress="return checkRemarks()" onBlur="return checkSpace(this)">'+other_specify+'</textarea></span></td>'+
								'</td>'+
								'</tr>'+
							 '<tr>'+
								'<td colspan="8">'+
									'<button type="button" style="margin-left: 400px;" class="btn btn-primary ven" onclick="ControlDivForVendorSelection(\'Save\',\'displayVendorSelection\',\'createVendorSelection\')">Save</button>'+
...
function DisplayQuotaionForVendorSelection()
{
	SessionCheck(function (ses){		
		if(ses)
			{
	...
	var searchWord = $('input[name="search"').val();
		$.post('Vendor_Selection',{action : 'Display',dt_frm:dt_frm,dt_to:dt_to,searchWord:searchWord},function (r){
		...
		if(r.data.length > 0)
			{
				...
				list = list + '<tr class="success">'+
									...
									'<td><strong><center><a class="alert" href="#" onclick="PreviewVendorSelection(\'createVendorSelection\',\'displayVendorSelection\',\''+params.no_quot+'\',\''+params.no_ind+'\',\''+params.id_quot+'\')"> Preview </a></center></strong></td>'+
									'</tr>';
				}
